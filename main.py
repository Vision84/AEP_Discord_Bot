...
def is_time_to_send(current_time, class_item):
    start_time, end_time, am_pm = re.findall(r'(\d:\d\d) - (\d:\d\d) ?(\w\w)', class_item)[0]
    class_start_time_str = f"{start_time} {am_pm}"
    class_start_time = datetime.strptime(class_start_time_str, "%I:%M %p")
    reminder_time = class_start_time - timedelta(hours=1)
    return current_time >= reminder_time.time()
